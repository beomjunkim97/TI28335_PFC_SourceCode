...
void Init_Code() {
	...
	//AD Reading
	data_ptr[ri++] = &theta_grid;	        // 0 100개의 DA변수 생성
	data_ptr[ri++] = &fdummy;           // 1 TEST버젼에서는 TEST1부터 TEST6까
	data_ptr[ri++] = &Va_grid;           // 2
 	data_ptr[ri++] = &Vb_grid;           // 3
	data_ptr[ri++] = &Vc_grid;           // 4
	data_ptr[ri++] = &fdummy;           // 5
	data_ptr[ri++] = &Ia_grid;           // 6
	data_ptr[ri++] = &Ib_grid;           // 7
	data_ptr[ri++] = &Ic_grid;           // 8
	data_ptr[ri++] = &fdummy;           // 9

	//Conversion
	data_ptr[ri++] = &theta_grid;           // 10
	data_ptr[ri++] = &Vqe_LPF;           // 11
	data_ptr[ri++] = &Vde_LPF;        // 12
	data_ptr[ri++] = &fdummy;           // 13
	data_ptr[ri++] = &Iqe_Grid;           // 14
	data_ptr[ri++] = &Ide_Grid;           // 15
	...
	//Current Controller
	data_ptr[ri++] = &Iqe_Grid_ref;           // 20
	data_ptr[ri++] = &Iqe_Grid;           // 21
	data_ptr[ri++] = &Ide_Grid_ref;           // 22
	data_ptr[ri++] = &Ide_Grid;           // 23 Vqse_ref
	data_ptr[ri++] = &fdummy;           // 24
	data_ptr[ri++] = &Vqe_ref;           // 25
	data_ptr[ri++] = &Vde_ref;           // 26
	data_ptr[ri++] = &fdummy;           // 27
	data_ptr[ri++] = &fdummy;           // 28
	data_ptr[ri++] = &fdummy;           // 29 Vqss_ref

	//Voltage Controller
	data_ptr[ri++] = &Vout_ref;           // 30
	data_ptr[ri++] = &Vdc;           // 31
	data_ptr[ri++] = &Vdc_LPF;           // 32
	data_ptr[ri++] = &Iqe_Grid_ref;           // 33
	data_ptr[ri++] = &Avg_Vdc;           // 34
...
void OutputDAC() {
	...
	dac_data0 = (int) ((DA_Data_ch0 - DA_Offset_ch0) * 2048 / DA_Scale_ch0)
			+ 0x800;  //(-)를 없애고 0부터 시작
	SpiaRegs.SPITXBUF = 0x0000 | (dac_data0 & 0x0fff);				//버퍼에 데이터 저장
	while (!SpiaRegs.SPISTS.bit.INT_FLAG)
		;											 //12bit 신호가 다 들어오는 것을 보장
	dac_temp = SpiaRegs.SPIRXBUF;							//버퍼에 있는 값을 temp에 저장

	dac_data1 = (int) ((DA_Data_ch1 - DA_Offset_ch1) * 2048 / DA_Scale_ch1)
			+ 0x800;
	...
	dac_data2 = (int) ((DA_Data_ch2 - DA_Offset_ch2) * 2048 / DA_Scale_ch2)
			+ 0x800;
	...
	dac_data3 = (int) ((DA_Data_ch3 - DA_Offset_ch3) * 2048 / DA_Scale_ch3)
			+ 0x800;
